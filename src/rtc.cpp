...
extern	UartComChan pc;			//	Serial Port
...
int32_t	get_date	()	{	//	return binary. From Hi to Lo bytes - 0x00, Year, Month, Date
	...
	RTC_DateTypeDef	rtc_date;
	HAL_RTC_GetDate	(&hrtc, &rtc_date, RTC_FORMAT_BIN);
	packed_date	|= (rtc_date.Year << 16);	//	Year binary in byte 2
	packed_date	|= (rtc_date.Month << 8);	//	Month binary in byte 1
	packed_date	|= (rtc_date.Date);			//	Date binary in byte 0
	return	(packed_date);
}
