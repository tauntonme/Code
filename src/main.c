...
//  spi_cs_set	();	//	No, this done by specifying initialise to '1' in .ioc setup

  HAL_TIM_Base_Start_IT(&htim6);	//	Need this to start timer
extern	void	start_uart_rx();

  start_uart_rx	();		//	Starts all used uarts
//  start_CAN_txrx	();	//	Now done as final step of init
...
*/
  ...
  while (1)
  {
	  ForeverLoop	();			//	Into 'C++' code, Never returns
  }
    /* USER CODE END WHILE */
